...
async def test_nijivoice(random_text, mp3_checker, audio_transcriber):
    resp_json = httpx.post(
        f"http://127.0.0.1:8000/nijivoice_encoded/api/platform/v1/voice-actors/{VOICE_ACTOR_ID}/generate-encoded-voice",
        json={
            "script": random_text,
            "speed": "1.0"
        }
    ).json()
    base64_audio = resp_json["generatedVoice"]["base64Audio"]
